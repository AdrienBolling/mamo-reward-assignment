"""Jitted random-policy rollout over vmapped environments (smoke/benchmark utility)."""

from __future__ import annotations

from typing import Any, NamedTuple

import jax
import jax.numpy as jnp

from mamora.envs.factory import CraftaxEnv

# MA-Craftax `EnvState` is a flax struct dataclass from an untyped package.
type EnvState = Any


class RolloutStats(NamedTuple):
    """Per-agent statistics averaged over the vmapped environments."""

    returns: jax.Array  # (num_agents,) sum of rewards over the rollout
    achievements: jax.Array  # (num_agents,) achievements unlocked at the end


def random_rollout(env: CraftaxEnv, key: jax.Array, *, num_envs: int, steps: int) -> RolloutStats:
    """Run `steps` uniform-random steps in `num_envs` parallel copies of `env`."""
    agents = tuple(env.agents)
    num_actions = int(env.action_space(agents[0]).n)

    def sample_actions(key: jax.Array) -> dict[str, jax.Array]:
        keys = jax.random.split(key, len(agents))
        return {
            agent: jax.random.randint(k, (num_envs,), 0, num_actions)
            for agent, k in zip(agents, keys, strict=True)
        }

    def step(
        carry: tuple[EnvState, jax.Array], _: None
    ) -> tuple[tuple[EnvState, jax.Array], jax.Array]:
        state, key = carry
        key, k_act, k_step = jax.random.split(key, 3)
        step_keys = jax.random.split(k_step, num_envs)
        _obs, state, rewards, _dones, _info = jax.vmap(env.step)(
            step_keys, state, sample_actions(k_act)
        )
        return (state, key), jnp.stack([rewards[agent] for agent in agents])

    @jax.jit
    def run(key: jax.Array) -> RolloutStats:
        k_reset, k_run = jax.random.split(key)
        _obs, state = jax.vmap(env.reset)(jax.random.split(k_reset, num_envs))
        (state, _), rewards = jax.lax.scan(step, (state, k_run), None, length=steps)
        # rewards: (steps, num_agents, num_envs); state.achievements: (num_envs, num_agents, n_ach)
        return RolloutStats(
            returns=rewards.sum(axis=0).mean(axis=-1),
            achievements=state.achievements.sum(axis=-1).mean(axis=0),
        )

    return run(key)
